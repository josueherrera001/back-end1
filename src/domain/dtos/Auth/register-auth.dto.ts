export class RegisterAuthDto{
    ...
    static create(props:{[key:string]:any}):[string?, RegisterAuthDto?]{
        const{UserName, UserPass,RoleId,UserId} = props;

        if ( !UserName )  return ['Debe ingresar el nombre del usuario',undefined];
        if ( !UserPass )  return ['Debe ingresar la contraseña del usuario',undefined];
        if ( UserPass < 8 && UserPass > 20 )  return ['La contraseña debe contener como minimo 8 y maximo 20 caracteres',undefined];
        // if ( !UserId )   return ['Debe ingresar el usuario',undefined]
        if ( !RoleId )  return ['Debe ingresar el tipo de aceso del usuario',undefined];
