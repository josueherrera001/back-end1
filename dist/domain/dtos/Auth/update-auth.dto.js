...
class UpdateAuthDto {
    ...
    static update(props) {
        ...
        if (OldUserPass)
            return [helpers_1.ErrorSpecific.ErrorEmpty('Debe debe ingresar la actual contraseña'), undefined];
        const isMatching = bcrypt_adapter_1.bcryptAdapter.compare(OldUserPass, ActiveUserPass);
        if (isMatching)
            return [helpers_1.ErrorSpecific.ErrorEmpty('La contraseña actual no es correcto'), undefined];
        if (UserPass)
            return [helpers_1.ErrorSpecific.ErrorEmpty('Debe ingresar la nueva contraseña del usuario'), undefined];
        if (UserPass < 8 && UserPass > 20)
            return [helpers_1.ErrorSpecific.ErrorEmpty('La contraseña nueva debe contener como minimo 8 y maximo 20 caracteres'), undefined];
