...
class RegisterAuthDto {
    ...
    static create(props) {
        ...
        if (!UserName)
            return [helpers_1.ErrorSpecific.ErrorEmpty('Debe ingresar el nombre del usuario'), undefined];
        if (!UserPass)
            return [helpers_1.ErrorSpecific.ErrorEmpty('Debe ingresar la contraseña del usuario'), undefined];
        if (UserPass < 8 && UserPass > 20)
            return [helpers_1.ErrorSpecific.ErrorEmpty('La contraseña debe contener como minimo 8 y maximo 20 caracteres'), undefined];
        // if ( !UserId )   return ['Debe ingresar el usuario',undefined]
        if (!RoleId)
            return [helpers_1.ErrorSpecific.ErrorEmpty('Debe ingresar el tipo de aceso del usuario'), undefined];
